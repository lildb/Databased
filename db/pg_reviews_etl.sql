/*
  To run this script, from the directory of this file:

    psql postgres -f ./pg_reviews_etl.sql

  ^^ Starts PostgreSQL in the default database "postgres" -
    You won't be able to DROP DATABASE if you're currently using it.

  Change the path to CSV files as appropriate below.
*/

/********** ------ WARNING! ------ **********/
/* Un-comment if you know what you're doing */
...
CREATE TEMP TABLE importreviews (
  review_id INT, product_id INT, rating SMALLINT,
  date BIGINT, summary TEXT, body TEXT,
  recommend BOOLEAN DEFAULT TRUE, reported BOOLEAN DEFAULT FALSE,
  reviewer_name TEXT, reviewer_email TEXT,
  response TEXT DEFAULT NULL, helpfulness SMALLINT DEFAULT 0
);
...
INSERT INTO list (
  ...
  body, response, recommend, reviewer_name,
  reviewer_email, helpfulness, reported
)
SELECT
  product_id,
  rating,
...
CREATE INDEX rp_idx ON list (product_id, review_id, date);
...
CREATE TEMP TABLE importphotos (id INT, review_id INT, url TEXT);
...
CREATE OR REPLACE VIEW photos_json AS
SELECT
  p.review_id,
  COALESCE (
    json_agg(
      json_build_object('id', p.id, 'url', url)
    ) FILTER (
      WHERE
        url IS NOT NULL
    ),
...
CREATE TEMP TABLE importspecs (
  id INT, product_id INT, name VARCHAR
);
...
CREATE TEMP TABLE importspecreviews (
  id BIGINT, characteristic_id INT, review_id INT,
  value INT
);
...
DROP
  TABLE importspecreviews;

CREATE INDEX s_avg ON spec_reviews (characteristic_id);

CREATE
OR REPLACE VIEW spec_avgs AS
SELECT
  sr.characteristic_id,
  avg(value) AS value
FROM
  spec_reviews sr
GROUP BY
...
CREATE
OR REPLACE VIEW product_specs AS
SELECT
  product_id,
  name,
  s.id
...
FROM
  product_specs ps
  FULL OUTER JOIN spec_avgs sa ON sa.characteristic_id = ps.id
ORDER BY
  product_id;
...
CREATE
OR REPLACE VIEW meta_specs_object AS
SELECT
  product_id,
  json_object_agg(name, averages) AS characteristics
FROM
  (
    SELECT
      json_build_object('id', id, 'value', value :: text) averages,
      product_id,
      name
    FROM
      (
...
FROM
  meta_rates rates
  RIGHT JOIN meta_recs recs ON rates.product_id = recs.product_id
  RIGHT JOIN meta_specs_object obj ON rates.product_id = obj.product_id;
