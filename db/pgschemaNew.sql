...
CREATE TABLE reviews.list (
  ...
  review_id INT GENERATED BY DEFAULT PRIMARY KEY,
...
-- CREATE INDEX r_p_idx ON reviews.list (product_id, review_id, reported); -- version 2 included reported as well
--index reviews by product id, then review id

CREATE INDEX rp_idx ON reviews.list (product_id, review_id, date, helpfulness); -- version 3 includes reported, to ignore search reported search results
...
GROUP BY product_id
ORDER BY product_id; --added 'recommended' functionality
